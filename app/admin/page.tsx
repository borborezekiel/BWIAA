...
function DonationsAdminTab({ donations, setDonations, showToast, isHeadAdmin, myChapter, adminEmail }: {
  donations: any[];
  setDonations: (value: any[] | ((prev: any[]) => any[])) => void;
  showToast: (m: string, ok?: boolean) => void;
  isHeadAdmin: boolean;
          ...
          <div className="bg-white rounded-[2rem] p-7 max-w-lg w-full shadow-2xl">
            ...
            {(selected.receipt_url || selected.photo_url) && (
              <div className="grid grid-cols-2 gap-3 mb-5">
                {selected.receipt_url && <a href={selected.receipt_url} target="_blank" className="bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl p-4 font-black text-xs uppercase text-center">View Receipt</a>}
                {selected.photo_url && <a href={selected.photo_url} target="_blank" className="bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl p-4 font-black text-xs uppercase text-center">View Photo</a>}
              </div>
            )}
            {selected.status === 'pending' && (
              <div className="grid grid-cols-2 gap-3">
...
function ContributionsAdminTab({ contributions, setContributions, showToast, isHeadAdmin, myChapter, adminEmail }: {
  contributions: any[];
  setContributions: (value: any[] | ((prev: any[]) => any[])) => void;
  showToast: (m: string, ok?: boolean) => void;
  isHeadAdmin: boolean;
  myChapter: string | null;
  adminEmail: string;
      ...
      {selected && (
        <div className="fixed inset-0 bg-slate-900/95 z-40 flex items-center justify-center p-4 backdrop-blur-md overflow-y-auto">
          <div className="bg-white rounded-[2rem] p-7 max-w-lg w-full shadow-2xl">
            ...
            {selected.receipt_url && <a href={selected.receipt_url} target="_blank" className="block bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl p-4 font-black text-xs uppercase text-center mb-5">View Receipt</a>}
            {selected.status === 'pending' && (
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => updateStatus(selected, 'approved')} disabled={processing} className="bg-green-600 hover:bg-green-700 text-white font-black uppercase py-4 rounded-2xl flex items-center justify-center gap-2 disabled:opacity-50">{processing ? <Loader2 size={16} className="animate-spin"/> : <CheckCircle2 size={16}/>} Approve</button>
                <button onClick={() => updateStatus(selected, 'rejected')} disabled={processing} className="bg-red-50 hover:bg-red-100 text-red-600 font-black uppercase py-4 rounded-2xl flex items-center justify-center gap-2 border-2 border-red-200 disabled:opacity-50"><XCircle size={16}/> Reject</button>
...
// ─── Portal Lock Panel ────────────────────────────────────────────────────────
function PortalLockPanel({ showToast }: { showToast: (m: string, ok?: boolean) => void }) {
  const [locked, setLocked]     = useState(false);
  const [message, setMessage]   = useState('The BWIAA election portal is currently closed. It will reopen for the next election cycle in 2028.');
  const [saving, setSaving]     = useState(false);
  const [loaded, setLoaded]     = useState(false);

  useEffect(() => {
    supabase.from('election_settings').select('key,value')
      .in('key', ['portal_locked','portal_locked_message'])
      .then(({ data }) => {
        if (!data) return;
        const get = (k: string) => data.find(r => r.key === k)?.value;
        setLocked(get('portal_locked') === 'true');
        if (get('portal_locked_message')) setMessage(get('portal_locked_message'));
        setLoaded(true);
      });
  }, []);
  ...
  async function save() {
    ...
    await supabase.from('election_settings').upsert([
      { key: 'portal_locked', value: String(locked) },
      { key: 'portal_locked_message', value: message },
    ], { onConflict: 'key' });
    setSaving(false);
    showToast(locked ? '🔒 Portal locked — members see the closed notice.' : '🔓 Portal unlocked — members have full access.');
  }

  if (!loaded) return <div className="flex items-center gap-2 py-2"><Loader2 size={14} className="animate-spin text-slate-400"/><p className="text-slate-400 font-bold text-sm">Loading...</p></div>;

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl">
        <div>
          <p className="font-black text-slate-800">{locked ? '🔒 Portal is LOCKED' : '🔓 Portal is OPEN'}</p>
          <p className="text-xs text-slate-400 font-bold mt-0.5">{locked ? 'Members see the closed notice below' : 'Members have full access to all features'}</p>
        </div>
        <button onClick={() => setLocked(!locked)}
          className={`relative w-14 h-7 rounded-full transition-all ${locked ? 'bg-red-500' : 'bg-green-500'}`}>
          <span className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-all ${locked ? 'left-8' : 'left-1'}`}/>
        </button>
      </div>

      <div>
        <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Closed Notice Message</label>
        ...
        <p className="text-xs text-slate-400 font-bold mt-1">Members see this message when the portal is locked.</p>
      </div>
      ...
      {locked && (
        <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-black text-red-700 uppercase tracking-widest">Portal Currently Locked</p>
            <p className="text-xs text-red-600 font-bold mt-1">Members can still log in but will see the closed notice. Admins retain full access. Toggle off to restore member access.</p>
          </div>
        </div>
      )}
