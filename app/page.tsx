...
export default function BWIAAElection2026() {
  ...
  // ── Whitelist / Blacklist check triggered after user is set ─────────────────
  useEffect(() => {
    if (!user) return;
  ...
  // ── Access check: HEAD ADMIN bypasses everything ─────────────────────────────
  async function checkAccess(email: string) {
    const lowerEmail = email.toLowerCase();

    // ★ HEAD ADMIN — unconditional bypass of all checks
    if (lowerEmail === HEAD_ADMIN_EMAIL.toLowerCase()) return;

    // ── 1. BLACKLIST — runs first, no exceptions for anyone else ──
    ...
    if (blocked) {
      ...
      setMyChapter(null);
      setErrorMessage(
        `ACCESS DENIED: This email has been blocked from this election. Reason: ${blocked.reason}. ` +
        `Contact the Election Committee if you believe this is an error.`
      );
      return;
    }

    // ── 2. WHITELIST — only enforced when roster has entries ──
    ...
    if (count && count > 0) {
      ...
      if (!found) {
        ...
        setMyChapter(null);
        setErrorMessage(
          `ACCESS DENIED: ${email} is not on the official BWIAA voter roster. ` +
          `Contact your chapter administrator to be added.`
        );
      }
    }
  }
  ...
  async function castBallot(pos: string, cand: string) {
    ...
    if (alreadyVoted) {
      setErrorMessage(`INTEGRITY ALERT: You have already cast a ballot for ${pos}. Each member may only vote once per position.`);
      return;
    }
  ...
  // LOCAL TALLY: filter votes to this voter's chapter for live per-candidate count
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20 text-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-4 space-y-12">
        {Object.entries(positionMap).map(([posTitle, candList]) => {
          ...
          return (
            <section
              key={posTitle}
              className={`bg-white p-8 md:p-12 rounded-[4rem] shadow-xl border-b-[18px] transition-all
                ${hasVoted ? 'border-green-400 opacity-80' : 'border-slate-200'}`}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {candList.map(cand => {
                  // LOCAL TALLY — counts only votes from this voter's chapter for live display
      ...
      {/* Audit Log Footer */}
      <footer className="max-w-4xl mx-auto mt-24 px-4 p-10 bg-slate-900 rounded-[3.5rem] text-white shadow-3xl relative overflow-hidden">
...
// ── Admin link: head admin always sees it; branch admins check election_admins ──
