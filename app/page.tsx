...
export default function BWIAAMasterBallot2026() {
  ...
  useEffect(() => {
    ...
    const live = supabase.channel('national-election')
  ...
  async function refreshVotes() {
    // This is the core fix: Pulling all votes so the tally is never 0
  ...
  async function registerAndLogin(chapter: string, classYear: string) {
    if (!classYear || classYear.length < 4) {
      setErrorMessage("Please enter a valid 4-digit Class Year.");
      return;
    }
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <header className="bg-white border-b-2 p-6 mb-10 sticky top-0 z-40 shadow-sm flex justify-between items-center max-w-5xl mx-auto rounded-b-[2.5rem]">
        <div className="flex items-center gap-3">
            ...
            <div className="font-black text-slate-900 uppercase leading-none">BWIAA<br/><span className="text-[10px] text-red-600">{myChapter} • CLASS OF {myClass || "VERIFIED"}</span></div>
        </div>
          ...
          <section key={pos.title} className="bg-white p-8 md:p-12 rounded-[4rem] shadow-xl border-b-[18px] border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pos.candidates.map(cand => {
                // IMPORTANT: Filter by chapter so local tallies are accurate
