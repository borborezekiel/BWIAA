...
export default function BWIAAElection2026() {
  ...
  // ADD ALL YOUR CANDIDATES HERE - IT WILL NOW AUTO-SCROLL
  const positions = [
    { title: "President", candidates: ["Candidate 1", "Candidate 2", "Candidate 3"] },
  ...
  async function castVote(pos: string, cand: string) {
    ...
    if (error) {
      setErrorMessage(`Integrity Check: You have already cast a ballot for the position of ${pos}.`);
    } else {
      setReceipt(data);
    }
  }
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white font-black italic">
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-6 md:p-12 flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-6xl w-full">
          {chapters.map(c => (
            <button key={c} onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } })} className="group bg-slate-900 border border-white/5 hover:border-red-600 p-8 rounded-[2.5rem] text-white font-bold transition-all flex flex-col items-center gap-4">
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <header className="bg-white border-b-2 p-6 mb-10 sticky top-0 z-40 shadow-sm">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black uppercase">{myChapter} BALLOT</h1>
          </div>
          <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-red-600 transition-colors">
      ...
      {receipt && (
        <div className="fixed inset-0 bg-slate-900/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white p-10 rounded-[3.5rem] max-w-md w-full text-center shadow-2xl border-t-[12px] border-green-500">
            ...
            <p className="text-slate-500 mb-8 font-medium">Recorded in Chapter: {receipt.chapter}</p>
      ...
      <main className="max-w-5xl mx-auto px-4 space-y-12">
        {positions.map(pos => (
          <section key={pos.title} className="bg-white p-8 md:p-12 rounded-[4rem] shadow-xl border-b-[18px] border-slate-200 overflow-hidden">
            <h2 className="text-3xl font-black text-slate-800 mb-10 uppercase italic tracking-tighter border-l-8 border-red-600 pl-6">{pos.title}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pos.candidates.map(cand => {
                ...
                return (
                  <button key={cand} onClick={() => castVote(pos.title, cand)} className="relative text-left p-8 rounded-[2.5rem] border-2 border-slate-50 hover:border-red-600 transition-all overflow-hidden group bg-slate-50/50 active:scale-95 shadow-sm">
                    <div className="relative z-10 flex justify-between items-center">
                        <span className="font-black text-xl group-hover:text-red-700">{cand}</span>
                        <span className="text-3xl text-red-600 font-black">{count}</span>
                    </div>
                    <div className="absolute left-0 top-0 h-full bg-red-100/40 -z-10 transition-all duration-1000 ease-out" style={{ width: `${percent}%` }} />
                  </button>
                );
              })}
            </div>
