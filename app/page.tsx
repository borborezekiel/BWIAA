...
export default function BWIAAElection2026() {
  ...
  async function checkWhitelist(email: string) {
    // If eligible_voters table has any rows, enforce whitelist.
    // If table is empty (open election mode), allow everyone.
    ...
    if (count && count > 0) {
      ...
      if (!found) {
        ...
        localStorage.clear();
        setUser(null);
        setErrorMessage(`ACCESS DENIED: The email ${email} is not on the official BWIAA voter roster. Contact your chapter administrator.`);
      }
    }
  }
